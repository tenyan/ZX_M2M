 ...
 * 构建包头
*************************************************************************/
int32_t GBEP_BuildMsgHead(uint8_t *pdata)
{
  //== GB环保数据公共部分 =========================================
...
int32_t GBEP_SendLogin(void)
{
  ...
  //== GB环保数据公共部分 =========================================
  retVal = GBEP_BuildMsgHead(pdata);
  if(retVal==-1)
  {
    return -1;
  }
...
void GBEP_BuildEngMessage(uint8_t* pdata)
{
  ...
  //=============================================================
  memcpy(pdata, ep_data_buffer, SIZE_OF_GBEP_ENG_MAND);
...
int32_t GBEP_SendBzData(void)
{
  ...
  if (stack_size > 0)
  {
    ...
    GjepBlindZone_Save(); // 存入栈参数
    if (bind_zone_data_size == 0x00)
    {
#if GBEP_BZ_DEBUG
      PcDebug_Printf("GjepBzPop:Err\r\n");
#endif
      return retVal;
    }
...
#if GBEP_BZ_DEBUG
  PcDebug_Printf("GjepBzInit:Ewr=%d,Erd=%d,Top=%d,Bot=%d\r\n",gbep_blind_zone.wr_error_cnt,gbep_blind_zone.rd_error_cnt,gbep_blind_zone.top,gbep_blind_zone.bottom);
#endif
}

//=====================================================================================
void GjepBlindZone_Save(void)
{
  uint16_t i;
...
void GjepBlindZone_Service(void)
{
      ...
      if (GBEP_GetLinkState() != SOCKET_LINK_STATE_READY) // 未连接
      {
        if ((GPS_GetPositioningStatus()==1) && (engine_speed>300)) // 终端已定位且发动机启动
        {
          ...
          if (gbep_blind_zone_length <= GBEP_BLIND_ZONE_PACKET_SIZE)
          {
            ...
            GjepBlindZone_Save(); // 存入栈参数
#if GBEP_BZ_DEBUG
            PcDebug_Printf("GjepBzPush:Ewr=%d,Erd=%d,Top=%d,Bot=%d\r\n",gbep_blind_zone.wr_error_cnt,gbep_blind_zone.rd_error_cnt,gbep_blind_zone.top,gbep_blind_zone.bottom);
#endif
          }
        }
      }
