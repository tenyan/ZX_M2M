 ...
 ******************************************************************************/
int fd_modem = -1;
static uint8_t modem_uart_rx_buffer[MODEM_UART_RX_BUFFER_SIZE];
...
const char AT_CSUB1[]="AT+CSUB=1\r\n"; // 查询模块二次开发版本
#define SIZE_OF_AT_CSUB1  (sizeof(AT_CSUB1)/sizeof(AT_CSUB1[0]) - 1)

const char AT_CSUB[]="AT+CSUB\r\n"; // 查询模块处理器型号
#define SIZE_OF_AT_CSUB  (sizeof(AT_CSUB)/sizeof(AT_CSUB[0]) - 1)
...
atStatus_t AT_SendCmd(at_cmd_t atcmdId, uint8_t retries, uint16_t timeout)
{
  ...
  case AT_CMD_CSUB1_GET: // 查询模块二次开发版本
    at_buf = AT_CSUB1;
    at_size = SIZE_OF_AT_CSUB1;
    break;

  case AT_CMD_CSUB_GET: // 查询模块处理器型号
    at_buf = AT_CSUB;
    at_size = SIZE_OF_AT_CSUB;
    break;
...
uint8_t Modem_Init(void)
{
  ...
  // 查询模块二次开发版本
  AT_SendCmd(AT_CMD_CSUB1_GET, 1, OS_TICKS_PER_SEC);

  // 查询模块处理器型号
  AT_SendCmd(AT_CMD_CSUB_GET, 1, OS_TICKS_PER_SEC);
  ...
  // 关闭WIFI模块
  at_rsp = AT_SendCmd(AT_CMD_CWMAP0_SET, 2, (OS_TICKS_PER_SEC*20));
  if (at_rsp == ATCMD_RSP_NOK)
  {
    //return AT_FALSE; // 退出重试
  }
...
static void Cellura_ProcessRecvData(uint8_t *pdata, uint16_t size)
{
  ...
  if(pdata[0]=='$')
  {
    ...
    nmeaMsgQueueENTER_CRITICAL();
    for (it = 0; it < size; it++)
    {
      nmea_msg_queue_push(pdata[it]); // 将收到的数据放入队列
    }
    nmeaMsgQueueEXIT_CRITICAL();
  ...
  if (urc_flag == CELLULAR_FALSE)
  {
    ...
    case AT_CMD_CGMR:
      ...
      break;
    case AT_CMD_CSUB1_GET:
      if (AT_RspAnalyzeURC(pdata, size, "OK"))
      {
        atcmd_rsp_status = ATCMD_RSP_OK;
      }
      break;
    case AT_CMD_CSUB_GET:
      if (AT_RspAnalyzeURC(pdata, size, "OK"))
      {
        atcmd_rsp_status = ATCMD_RSP_OK;
      }
