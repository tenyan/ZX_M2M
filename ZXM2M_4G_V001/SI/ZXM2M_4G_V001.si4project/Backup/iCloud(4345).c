...
void* pthread_SubEpProcess(void *argument)
{
  while (1)
  {
    if ((NetSocket_GetLinkState(&subep_socket)==SOCKET_LINK_STATE_CLOSED) || (HjepSocketFd<0))
    {
      msleep(10);
    }
    else
...
{
  // 环保主连接
  pthread_create(&pthreads[PTHREAD_MAINEP_PRODUCE_ID], NULL, pthread_MainEpProduce, NULL);
  usleep(10);
  pthread_create(&pthreads[PTHREAD_MAINEP_PROCESS_ID], NULL, pthread_MainEpProcess, NULL);
  usleep(10);
  pthread_create(&pthreads[PTHREAD_MAINEP_SOCKET_SERVICE_ID], NULL, pthread_MainEpSocketService, NULL);
  usleep(10);

  // 环保副连接
  //pthread_create(&pthreads[PTHREAD_SUBEP_PRODUCE_ID], NULL, pthread_SubEpProduce, NULL);
  //usleep(10);
  pthread_create(&pthreads[PTHREAD_SUBEP_PROCESS_ID], NULL, pthread_SubEpProcess, NULL);
  usleep(10);
  pthread_create(&pthreads[PTHREAD_SUBEP_SOCKET_SERVICE_ID], NULL, pthread_SubEpSocketService, NULL);
  usleep(10);
}
