...
//==转发M2M的PW/PR/RC指令给ST
#define AuxCom_SendM2mData AUX_UartTransmitData
...
typedef enum
{
  FSRV_COMMAND_TYPE_NOTIFIE = 0x01,  // 升级通知
  FSRV_COMMAND_TYPE_DOWNLOAD = 0x02,  // 下载数据
  FSRV_COMMAND_TYPE_REPORT = 0x03,  // 上报结果
}fsrv_command_t;
...
typedef enum
{
  ...
  FSRV_STATE_SEND_UN_REQ,  /// 发送通知升级
  FSRV_STATE_WAIT_REQ,     /// 等待ST请求和响应
  FSRV_STATE_SEND_UD_RSP,  /// 发送升级数据(响应ST请求)
  FSRV_STATE_SEND_UR_RSP,  /// 发送升级结果(响应ST请求)
...
typedef struct 
{
  ...
  uint16_t timeout_50ms_sp;  // 超时设定值
  uint16_t timeout_50ms;  // 超时计时器
