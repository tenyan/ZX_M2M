...
// ERRCHKstatic const uint8_t DAYS_OF_MONTH[] = {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
...
void mcu_i2c_write(uint8 len, uint8 address, uint8 *wrptr)
{
  ...
  if(err != OS_ERR_NONE) { return;}
	...
	i2c_write_byte((AM1805_I2C_ADDR & 0xFE)); // Write Control Byte MAKE SURE IT IS WRITE CONDITON
  i2c_wait_ack();
	i2c_write_byte(address);             // send low address
  i2c_wait_ack();
	for(i=0;i<len;i++)
	{
...
void mcu_i2c_read(uint8 len, uint8 address, uint8* data)
{
  ...
  i2c_wait_ack();
	...
	i2c_write_byte((AM1805_I2C_ADDR | 0x01)); // Write control byte for read
  i2c_wait_ack();
...
////////////////////////////////////////////////////////////////////////////////
// Use SPI versions of low-level AMX8XX register access routines.
...
void Am_SPI_Config(void)
{
	...
	/* Connect PA5 to SPI2_SCK */
	GPIO_PinAFConfig(GPIOA, GPIO_PinSource5, GPIO_AF_SPI1);
	/* Connect PA6 to SPI2_MOSI */
	GPIO_PinAFConfig(GPIOA, GPIO_PinSource6, GPIO_AF_SPI1);
	/* Connect PA7 to SPI2_MISO */
...
void mcu_spi_read(uint8 len, uint8 addr, uint8 *dat)
{
	uint8 i;
	...
	for (i=0; i<len; i++)
	{
		*(dat+i) = SPI1_SendByte(0xa5);
	}
	SPI1_CS_H();
}

void mcu_spi_write(uint8 len, uint8 addr, uint8 *dat)
{
	uint8 i;
...
void clrreg(uint8_t address, uint8_t mask)
{
	...
	mcu_spi_read(1, address, &temp);
	temp &= ~mask;
	mcu_spi_write(1, address, &temp);
}
...
// Executes SPI commands required to read one byte of data at a particular address
uint8_t readreg(uint8_t address)
{
	uint8_t reg_data;

	mcu_spi_read(1,address, &reg_data);
	return reg_data;
}


// Executes SPI commands required to write one byte of data to a particular address
void writereg(uint8_t address, uint8_t data)
{
	mcu_spi_write(1, address, &data);
}


// Executes SPI commands required to implement a burst read of the data at a particular
// address of a particular length into a structure
void burstread(uint8_t address, uint8_t length, uint8_t * data)
{
	mcu_spi_read(length, address, data);
}


// Executes SPI commands required to implement a burst write of the data in a structure to
// a particular address with a particular length
void burstwrite(uint8_t address, uint8_t length, uint8_t * data)
{
	mcu_spi_write(length, address, data);
}

#endif

// If the OUT2S field is set to the initial value of 7, 
// the PSW/nIRQ2 pin will be driven with the value of the OUTB bit which is initially zero.
// A static value which may be driven on the PSW/nIRQ2 pin. 
// The OUTB bit cannot be set to 1 if the LKO2 bit is 1.
void PSWControl(uint8 state)
{
//	uint8 temp = 0;

	clrreg(OSC_STATUS_REG, 0x20);  // unlock OUT2

	clrreg(CONTROL_2_REG, 0x1C);  	// Clear OUT2S  OUT2S=000
	setreg(CONTROL_2_REG, 0x1C);  	// Set OUT2S to 7  OUT2S=111
	if (0==state)
	{
		//clrreg(CONTROL_2_REG, 0x1C);  	// Clear OUT2S
		//setreg(CONTROL_2_REG, 0x1C);  	// Set OUT2S to 7
		clrreg(CONTROL_1_REG, 0x20);      // OUTB = 0
	}
	else
	{
		//clrreg(CONTROL_2_REG, 0x1C);  	// Clear OUT2S
		//setreg(CONTROL_2_REG, 0x0C);  	// Set OUT2S to 6
		setreg(CONTROL_1_REG, 0x20);      // OUTB = 1
	}
}
...
void am_jhm(uint8 mode)
{
	uint8 temp1;
	/****intial**********/
	if (readreg(INT_MASK_REG)&0x04==0) //Alarm if not enable,RTC is first time power on
	{
		printf("rtc need initial \n \r");
		//*********set time****************************//
		time_regs.hundredth=0;
		time_regs.second=21;
		time_regs.minute=50;
		...
		time_regs.year=15;
		time_regs.century=0;
		time_regs.mode=2;//24 hour
		temp1=am_set_time(1);//time can't  rewritable
		if (temp1==SUCCESS)
		{
		...
		time_regs.weekday=3;
		time_regs.month=7;
		am_set_alarm(5,0,1);//set alarm  fout per hour
		printf("current alarm is %x-%x-%x s \n \r ", time_regs.hour,time_regs.minute,time_regs.second);

		//**************intial poweron nubmer register**********//
		writereg(0x40,0);//intial poweron nubmer=0
		writereg(0x41,0);//intial poweron nubmer=0
	}
	while (1)
	{
		if (readreg(STATUS_REG)&0x04!=0)
			clrreg(STATUS_REG, 0x04);//clear alarm interrupter flag
		if (mode==1) //Fout mode for 1 hour interrupter
		{
			time_regs.hundredth=0;
			time_regs.second=41;
			time_regs.minute=50;
		...
		else if (mode==2) //PSW mode for 2 hour interrupter
		{
			time_regs.hundredth=0;
			time_regs.second=41;
			time_regs.minute=50;
			...
			temp1=am_set_sleep(2, 1);//2*7.8ms
			if (temp1==SLEEP_SUCCESS)
			{
				printf("Sleep Success! \n \r");
			}
			else if (temp1==SLEEP_NO_INT_ENABLED)
			{
				printf("Sleep no interruper enable! \n \r");
				time_regs.second=41;
				time_regs.minute=50;
				time_regs.hour=15;
				time_regs.date=1;
			...
			else if (temp1==SLEEP_INT_PENDING)
			{
				clrreg(STATUS_REG, 0x7F);//clr interrupter flag
				printf("Sleep interrupter pending! \n \r");
				temp1=am_set_sleep(2, 1);
			}
		}
	}
}
