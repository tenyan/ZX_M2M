...
// 测试用
//#define AuxCom_Transmit(pdata, size)    do{USART6_TransmitData(pdata, size);PcDebug_SendData(pdata, size, DBG_MSG_TYPE_ANYDATA);}while(0);
...
#define FCLIT_TIMEOUT_SP          100  // 基于50ms
 ...
 ******************************************************************************/
//=消费者==============================================================
#define AppThreadPriority_AuxComProcess   osPriorityAboveNormal2
...
//=生产者==============================================================
#define AppThreadPriority_AuxComProduce   osPriorityAboveNormal1
...
void FCLIT_Do50msTasks(void)
{
  ...
  //==超时重试判断===========================================
  if(fclit_context.timeout_50ms)
  {
    fclit_context.timeout_50ms--;
    if(fclit_context.timeout_50ms==0x00)
    {
...
void FCLIT_Init(void)
{
  ...
  fclit_context.timeout_50ms_sp = FCLIT_TIMEOUT_SP;
  fclit_context.timeout_50ms = 0x00;
}
...
{
#if FCLIT_DEBUG
  PcDebug_Printf("Rfu:Erase!\n");
#endif
  
  rfu_EraseFlashHexFile(pThis->dev_id);  // 擦除Flash
  pThis->retry_cnt = 0;
  pThis->ud_result = 0x01;  // 未成功
  pThis->timeout_50ms = pThis->timeout_50ms_sp;

  pThis->cumulated_address = 0;
  pThis->fw_packet_index = 0x00;
  pThis->total_block_count = rfu_context.total_block_count;
...
void iFCLIT_SendUdReqMsg(fclit_context_t* pThis)
{
  ...
  // 7E 00 06 84 01 02 00 00 87 0D 0A
  pThis->msg_sn++;
  pThis->retry_cnt++;
  pdata[FCLIT_FRAME_STX_FIELD] = FCLIT_MSG_HEAD;  // 帧头(1B)
  pdata[FCLIT_FUNCTION_CODE_FIELD] = FCLIT_SEND_COMMAND_ID;  // 命令号
...
void FCLIT_ProduceSendMsg(fclit_context_t* pThis)
{
  ...
  switch (pThis->state)
  {
  case FCLIT_STATE_SEND_UN_RSP: /// 向4G发送升级通知响应
...
void iFCLIT_AnalyzeUnReqMsg(fclit_context_t* pThis)
{
  ...
  pThis->state = FCLIT_STATE_SEND_UN_RSP;  // 状态跳转(发送升级通知响应)
...
void iFCLIT_AnalyzeUdRspMsg(fclit_context_t* pThis)
{
  ...
  if (current_block_index != pThis->fw_packet_index)
  {
    return;
  }
  ...
  if (total_block_num != pThis->total_block_count)
  {
    return;
  }
  ...
  rfu_SaveFlashHexFile(pThis->dev_id, pThis->cumulated_address, &pdata[pos], packet_len);  // 将BIN文件写入SPI FLASH
  pThis->cumulated_address += packet_len;
  ...
  if (current_block_index==(total_block_num-1)) // 传输结束(最后一包)
  {
    ...
    {
      pThis->ud_result = 0;
      if (pThis->dev_id==0x01) // 控制器
      {
        PcDebug_SendString("RfuCrc:CTL-Ok!\n");
      }
      else if (pThis->dev_id==0x02) // 显示器
      {
        PcDebug_SendString("RfuCrc:LCD-Ok!\n");
      }
      else if (pThis->dev_id==0x03) // 终端协处理器
      {
...
void AppTask_AuxComProduce(void *argument)
{
  while (1)  // 100m执行一次
  {
    AUX_DELAY_MS(OS_TICKS_PER_SEC/20); // 50ms执行一次
    FCLIT_ProduceSendMsg(&fclit_context); // 发送AUX消息
  }
}
