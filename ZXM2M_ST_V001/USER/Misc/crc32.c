...
/*
  ...
  Revert: true
  XorOut: 0xFFFFFFFF
  Check : 0xCBF43926 ("123456789")  0x8DA988AF("abcdefghi")
...
/*************************************************************************
 * crc32val初始值是0xFFFFFFFF
*************************************************************************/
uint32_t GetCrc32(const void* data, uint16_t length)
{
	uint16_t i;
  const uint8_t *buf = (uint8_t *) data; //Point to the data over which to calculate the CRC
	uint32_t crc32val = 0xFFFFFFFF; // 余数初始值   Initial value
	...
	return (crc32val ^ 0xFFFFFFFF); // 结果异或值  Final XOR value
}
