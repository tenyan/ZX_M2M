...
void ZxStsEngine_Initialize(zxsts_engine_context_t* pThis)
{
  ...
  pThis->twt_up = TCW_NUM_DWORD((zxup_buffer_a5c8+ZXUP_A5C8_POS1));  // 作业总运行时间
  pThis->tfc_up = TCW_NUM_DWORD((zxup_buffer_a5c8+ZXUP_A5C8_POS2));  // 作业燃油总消耗量
  pThis->afc_up = TCW_NUM_WORD((zxup_buffer_a5c8+ZXUP_A5C8_POS3));  // 作业平均油耗

  //从EEPROM内获取行驶油耗统计
  Parm_ReadZxEngineDwStsInfo();
  pThis->twt_down = TCW_NUM_DWORD((zxengine_buffer_a5f0+ZXENGINE_A5F0_POS1));  // 行驶总运行时间
  pThis->tfc_down = TCW_NUM_DWORD((zxengine_buffer_a5f0+ZXENGINE_A5F0_POS2));  // 行驶燃油总油耗量
  pThis->afc_down = TCW_NUM_DWORD((zxengine_buffer_a5f0+ZXENGINE_A5F0_POS3));  // 百公里油耗
