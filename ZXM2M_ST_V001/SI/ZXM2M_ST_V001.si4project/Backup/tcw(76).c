...
//==设置开始计时标志==========================================================
void ZxStsEngine_SetEngineType(zxsts_engine_context_t* pThis, uint8_t EngineType)
{
  pThis->engine_type = EngineType; // 单发动机或双发动机
}

typedef struct
{
  uint8_t engine_type;  // 发动机类型: 单发=0x00, 双发=0x01
...
void ZxStsEngine_StateMachine(zxsts_engine_context_t* pThis)
{
  ...
  static uint8_t divide_for_10min = 59;
  static uint8_t DO_1MIN_FLAG = ZXSTS_FALSE;
  static uint8_t DO_10MIN_FLAG = ZXSTS_FALSE;

  acc_state = COLT_GetAccStatus();
  engine_speed = CAN_GetEngineSpeed();
  pto_flag.byte = zxdown_buffer_a5e7[ZXDOWN_A5E7_POS1]; // 动力系统

  //==确认工作状态=======
  if ((acc_state==1) && (engine_speed > 300)) // ACC开和发动机发动
  {
    if (pThis->engine_type=0x00) // 单发=0x00
    {
    ...
    else if (pThis->engine_type=0x01)// 双发=0x01
    {
  ...
  if ((acc_state==1) && (engine_speed > 300)) // ACC开和发动机发动
  {
    if (!divide_for_1min)
    {
      ...
      if (!divide_for_10min)
      {
        DO_10MIN_FLAG = ZXSTS_TRUE;
        DO_10MIN_FLAG = 9;
      }
      else divide_for_10min--;
    }
  ...
  if (DO_1MIN_FLAG==ZXSTS_TRUE)
  {
    ...
    if (pThis->up_work_flag)
    {
      ...
      pThis->tfc_up += pThis->fdv_up;  // 累计工时
...
//==初始化发动机工作统计参数=================================================
void ZxStsEngine_Initialize(void)
{
  uint8_t it;
