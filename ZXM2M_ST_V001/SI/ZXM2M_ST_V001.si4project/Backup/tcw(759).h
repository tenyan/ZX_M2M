...
#define TCW_SPLIT_DWORD(pbuf, dword) do{ pbuf[0]=(uint8_t)(dword>>24); pbuf[1]=(uint8_t)(dword>>16); pbuf[2]=(uint8_t)(dword>>8); pbuf[3]=(uint8_t)(dword);}while(0)
...
typedef struct
{
  uint32_t total_slept_time;
  uint32_t current_slept_time;
}tboxsts_context_t;
extern tboxsts_context_t tboxsts_context;
