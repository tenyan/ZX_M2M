...
*
* 扇区大小与块大小
* FlashDB 内部存储结构由N个扇区组成，每次格式化时是以扇区作为最小单位。
* 而一个扇区通常是 Flash 块大小的 N 倍，比如： Nor Flash 的块大小一般为 4096。
* 默认 KVDB 会使用 1倍的块大小作为扇区大小，即：4096。此时，该 KVDB 无法存入超过 4096 长度的 KV 。
* 如果想要存入比如：10K 长度的 KV ，可以通过 control 函数，设置扇区大小为 12K，或者更大大小即可。
******************************************************************************/
#include <string.h>
#include "cmsis_os2.h"
...
#define SECTOR_HDR_DATA_SIZE  (FDB_WG_ALIGN(sizeof(sector_hdr_data_t)))
#define SECTOR_DIRTY_OFFSET  ((unsigned long)(&((sector_hdr_data_t *)0)->status_table.dirty))
...
#ifdef FDB_KV_USING_CACHE
  uint32_t empty_kv;
  if (iFDB_GetSectorFromCache(db, FDB_ALIGN_DOWN(start, FDB_SECTOR_SIZE(db)), &empty_kv) && start == empty_kv)
  {
    return FAILED_ADDR;
  }
#endif
...
//====================================================================================
static uint32_t iFDB_GetNextKvAddr(fdb_kvdb_t* db, kvdb_sec_info_t* sector, fdb_kv_t* pre_kv)
{
  ...
  else
  {
    if (pre_kv->addr.start <= sector->addr + FDB_SECTOR_SIZE(db))
    {
      ...
      addr = iFDB_FindNextKvAddr(db, addr, sector->addr + FDB_SECTOR_SIZE(db));  // check and find next KV address
...
static fdb_err_t iFDB_ReadKv(fdb_kvdb_t* db, fdb_kv_t* kv)
{
  ...
  else if (kv->len > FDB_SECTOR_SIZE(db) - SECTOR_HDR_DATA_SIZE && kv->len < FDB_MAX_SIZE(db))
  {
...
//====================================================================================
static fdb_err_t iFDB_ReadSectorInfo(fdb_kvdb_t* db, uint32_t addr, kvdb_sec_info_t* sector, bool traversal)
{
  fdb_err_t result = FDB_NO_ERR;
  sector_hdr_data_t sec_hdr = { 0 };

  FDB_ASSERT(addr % FDB_SECTOR_SIZE(db) == 0);
  FDB_ASSERT(sector);

  iFDB_FlashRead((fdb_db_t*)db, addr, (uint32_t *)&sec_hdr, sizeof(sector_hdr_data_t));  // read sector header raw data
  ...
  if (traversal)  // traversal all KV and calculate the remain space size
  {
    ...
    if (sector->status.store == FDB_SECTOR_STORE_EMPTY)
    {
      sector->remain = FDB_SECTOR_SIZE(db) - SECTOR_HDR_DATA_SIZE;
    }
...
//====================================================================================
static uint32_t iFDB_GetNextSectorAddr(fdb_kvdb_t* db, kvdb_sec_info_t* pre_sec)
{
  uint32_t next_addr;
...
static fdb_err_t iFDB_FormatSentor(fdb_kvdb_t* db, uint32_t addr, uint32_t combined_value)
{
  ...
  if (result == FDB_NO_ERR)
  {
    ...
    memset(&sec_hdr, 0xFF, sizeof(sector_hdr_data_t));
    ...
    result = iFDB_FlashWrite((fdb_db_t*)db, addr, (uint32_t *)&sec_hdr, sizeof(sector_hdr_data_t));  // save the header

#ifdef FDB_KV_USING_CACHE
    iFDB_UpdateSectorCache(db, addr, addr + FDB_SECTOR_SIZE(db));  // delete the sector cache
#endif
  }
...
//====================================================================================
static fdb_err_t iFDB_UpdateSentorStatus(fdb_kvdb_t* db, kvdb_sec_info_t* sector, size_t new_kv_len, bool *is_full)
{
...
#ifdef FDB_KV_USING_CACHE
      iFDB_UpdateSectorCache(db, sector->addr, sector->addr + FDB_SECTOR_SIZE(db));  // delete the sector cache
#endif
...
//====================================================================================
static void iFDB_SentorIterator(fdb_kvdb_t* db, kvdb_sec_info_t* sector, fdb_sector_store_status_t status, void *arg1, void *arg2,
...
//====================================================================================
static bool iFDB_SentorStatisticsCb(kvdb_sec_info_t* sector, void *arg1, void *arg2)
{
...
//====================================================================================
static bool iFDB_AllocKvCb(kvdb_sec_info_t* sector, void *arg1, void *arg2)
{
...
//====================================================================================
static uint32_t iFDB_AllocKv(fdb_kvdb_t* db, kvdb_sec_info_t* sector, size_t kv_size)
{
...
static fdb_err_t iFDB_DelKv(fdb_kvdb_t* db, const char *key, fdb_kv_t* old_kv, bool complete_del)
{
  ...
  else
  {
    ...
    if (!last_is_complete_del && result == FDB_NO_ERR)
    {
      ...
      {
        // when using iFDB_DelKv(db, key, NULL, true) or iFDB_DelKv(db, key, kv, true) in fdb_del_kv(db, ) and iFDB_SetKv(db, )
  ...
  dirty_status_addr = FDB_ALIGN_DOWN(old_kv->addr.start, FDB_SECTOR_SIZE(db)) + SECTOR_DIRTY_OFFSET;
...
#ifdef FDB_KV_USING_CACHE
    iFDB_UpdateSectorCache(db, FDB_ALIGN_DOWN(kv_addr, FDB_SECTOR_SIZE(db)), (kv_addr + KV_HDR_DATA_SIZE + FDB_WG_ALIGN(kv->name_len) + FDB_WG_ALIGN(kv->value_len)));
...
//====================================================================================
static uint32_t iFDB_NewKv(fdb_kvdb_t* db, kvdb_sec_info_t* sector, size_t kv_size)
{
...
//====================================================================================
static uint32_t iFDB_NewKvEx(fdb_kvdb_t* db, kvdb_sec_info_t* sector, size_t key_len, size_t buf_len)
{
...
//====================================================================================
static bool iFDB_GcCheckCb(kvdb_sec_info_t* sector, void *arg1, void *arg2)
{
...
//====================================================================================
static bool iFDB_DoGc(kvdb_sec_info_t* sector, void *arg1, void *arg2)
{
  fdb_kv_t kv;
  fdb_kvdb_t* db = arg1;
...
static void iFDB_GcCollect(fdb_kvdb_t* db)
{
  ...
  iFDB_SentorIterator(db, &sector, FDB_SECTOR_STORE_EMPTY, &empty_sec, NULL, iFDB_GcCheckCb, false);  // GC check the empty sector number
...
//====================================================================================
static fdb_err_t iFDB_CreatKvBlob(fdb_kvdb_t* db, kvdb_sec_info_t* sector, const char *key, const void *value, size_t len)
{
  fdb_err_t result = FDB_NO_ERR;
  kv_hdr_data_t kv_hdr;
  bool is_full = false;
  ...
  if (kv_hdr.len > FDB_SECTOR_SIZE(db) - SECTOR_HDR_DATA_SIZE)
  {
...
//====================================================================================
static fdb_err_t iFDB_SetKv(fdb_kvdb_t* db, const char *key, const void *value_buf, size_t buf_len)
{
  fdb_err_t result = FDB_NO_ERR;
  static fdb_kv_t kv;
  static kvdb_sec_info_t sector;
...
fdb_err_t FlashDB_KvSetBlob(fdb_kvdb_t* db, const char *key, fdb_blob_t* blob)
{
  ...
  FDB_LOCK(db);  // lock the KV cache
  result = iFDB_SetKv(db, key, blob->buf, blob->size);
  FDB_UNLOCK(db);  // unlock the KV cache
...
static void iFDB_KvAutoUpdate(fdb_kvdb_t* db)
{
  ...
  iFDB_SetKv(db, VER_NUM_KV_NAME, &setting_ver_num, sizeof(size_t));
}
#endif // FDB_KV_AUTO_UPDATE

//====================================================================================
static bool iFDB_CheckSecHdrCb(kvdb_sec_info_t* sector, void *arg1, void *arg2)
{
  if (!sector->check_ok)
  {
...
//====================================================================================
static bool iFDB_CheckAndRecoveryGcCb(kvdb_sec_info_t* sector, void *arg1, void *arg2)
{
  fdb_kvdb_t* db = arg1;
