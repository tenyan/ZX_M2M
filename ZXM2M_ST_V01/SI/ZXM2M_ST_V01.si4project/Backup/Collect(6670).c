...
void COLT_ManageLedStatus(void)
{
  ...
  else if(tbox_state==TBOX_STATE_WORKING)
  {
    ...
    //==黄灯(CAN)=================================================
    if(CAN_GetCommState(CAN_CHANNEL1)==CAN_OK && CAN_GetCommState(CAN_CHANNEL2)==CAN_OK)
    {
      led_control[LED_CAN].blink = FAST_CONTINUOUS;
    }
    else if(CAN_GetCommState(CAN_CHANNEL1)==CAN_NOK && CAN_GetCommState(CAN_CHANNEL2)==CAN_NOK)
    {
      led_control[LED_CAN].blink = STEADY_OFF;
    }
    else
