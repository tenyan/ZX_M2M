...
void rtcSoft_Update(void)
{
  ...
  if(st_time.ms_temp > 999) // 计时满1秒
  {
    st_time.ms_temp -= 1000;
    st_time.ms = st_time.ms_temp;

    st_time.second++;
    if(st_time.second > 59) // 计时满1分钟
    {
      st_time.second = 0;
      st_time.minute++;
      if(st_time.minute > 59) // 计时满1小时
      {
        st_time.minute = 0;
        st_time.hour++;
        if(st_time.hour > 23) // 计时满1天
        {
          st_time.hour = 0;
