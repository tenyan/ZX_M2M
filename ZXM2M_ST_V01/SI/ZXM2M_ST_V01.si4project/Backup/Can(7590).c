...
/**********************************************************************************
* 检查MCU通信状态是否改变，此函数1s执行一次
***********************************************************************************/
void iCan_CheckCommEvent()
{
  static uint8_t comm_previous_state1 = 0;
  static uint8_t comm_previous_state2 = 0;
  static uint8_t recv_previous_state1 = 0;
  static uint8_t recv_previous_state2 = 0;

  // CAN1通信状态变化监测
  if (can_context.comm_state1 != comm_previous_state1) // 状态跳变
  {
    if (can_context.comm_state1==CAN_OK) // 正常
    {
      SbusMsg_Can.type = CAN_MSG_TYPE_CAN1_COMM_OK;
    }
    else // 异常
    {
      SbusMsg_Can.type = CAN_MSG_TYPE_CAN1_COMM_ERR;
    }
    SYSBUS_PutMbox(SbusMsg_Can);
  }
  comm_previous_state1 = can_context.comm_state1;

  // CAN2通信状态变化监测
  if (can_context.comm_state2 != comm_previous_state2) // 状态跳变
  {
    if (can_context.comm_state2==CAN_OK)  // 正常
    {
      SbusMsg_Can.type = CAN_MSG_TYPE_CAN2_COMM_OK;
    }
    else // 异常
    {
      SbusMsg_Can.type = CAN_MSG_TYPE_CAN2_COMM_ERR;
    }
    SYSBUS_PutMbox(SbusMsg_Can);
  }
  comm_previous_state2 = can_context.comm_state2;

  // CAN1接收状态变化监测
  if (can_context.recv_state1 != recv_previous_state1) // 状态跳变
  {
    if (can_context.recv_state1==CAN_OK) // 通信开始
    {
      SbusMsg_Can.type = CAN_MSG_TYPE_CAN1_RECV_START;
    }
    else // 通信中断
    {
      SbusMsg_Can.type = CAN_MSG_TYPE_CAN1_RECV_STOP;
    }
    SYSBUS_PutMbox(SbusMsg_Can);
  }
  recv_previous_state1 = can_context.recv_state1;

  // CAN2接收状态变化监测
  if (can_context.recv_state2 != recv_previous_state2) // 状态跳变
  {
    if (can_context.recv_state2==CAN_OK) // 通信开始
    {
      SbusMsg_Can.type = CAN_MSG_TYPE_CAN2_RECV_START;
    }
    else // 通信中断
    {
      SbusMsg_Can.type = CAN_MSG_TYPE_CAN2_RECV_STOP;
    }
    SYSBUS_PutMbox(SbusMsg_Can);
  }
  recv_previous_state2 = can_context.recv_state2;
}
...
void iCan_CheckCommState(void)
{
  ...
  if (acc_current_state==1) // ACC打开
  {
    ...
    if ((acc_on_timer==0) && (can_context.recv_state1==0) && (reset_can_flag==0))
    {
      reset_can_flag = 1;
      reset_can_timer = 5;
...
void CAN1_ProcessRecvMsg(can_msg_t* pMsg)
{
  ...
  retval = CAN_ProcessRecvDownMsg(pMsg->id, pMsg->data, pMsg->len); // 下车底盘数据
  if (retval==0x01)
  {
    return;
  }

  retval = CAN_ProcessRecvEngineMsg(pMsg->id, pMsg->data, pMsg->len); // 下车发动机数据
  if (retval==0x01)
  {
    return;
  }
