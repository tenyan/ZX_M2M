...
void Modem_PowerOn(void)
{
  ...
  MODEM_DELAY_MS(OS_TICKS_PER_SEC*3); // 延时3s进入数据发送
}
...
void iMomi_BuildAdSwitch(void)
{
  ...
  zxinfo_buffer_a5ff[ZXINFO_A5FF_POS6_ADDR] = colt_info.alarm; // 防拆类开关量
  zxinfo_buffer_a5ff[ZXINFO_A5FF_POS7_ADDR] = tbox_mode; // 工作模式

  tempVal = HW_VERSION;
  zxinfo_buffer_a5ff[ZXINFO_A5FF_POS8_ADDR] = (tempVal>>8) & 0xFF;  // 版本信息
  zxinfo_buffer_a5ff[ZXINFO_A5FF_POS8_ADDR+1] = (tempVal & 0xFF);
...
uint8_t Momi_CheckMsg(uint8_t *pdata, uint16_t size)
{
  ...
  calculated_check_sum = iMomi_CalcSumCheck(&pdata[3], msg_len); // 计算校验值:从命令号开始计算
