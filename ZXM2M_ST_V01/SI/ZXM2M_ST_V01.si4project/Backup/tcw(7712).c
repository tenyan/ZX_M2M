...
//==累计工作时间=============================================================
void zxsts_AccumulateTime(zxsts_context_t* pThis)
{
  //==CAN帧消失去耦时间==========================
  if(pThis->debounce_timer)
    pThis->debounce_timer--;
  else
  {
    pThis->work_flag = 0x00;
  }

  //==时间累计===================================
  if(pThis->work_flag==0x01)  // 设备工作中
  {
    pThis->timer_1s++;  // 时间累计
    if(pThis->timer_1s >= ZXSTS_STEP_SP)
    {
      pThis->timer_1s -= ZXSTS_STEP_SP;
...
void zxsts_StateMachine(void)
{
  ...
  {
    zxsts_AccumulateTime(&zxsts_context[it]);  // 作业统计
  }
}
