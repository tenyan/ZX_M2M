...
uint8_t CAN_ProcessRecvDownMsg(uint32_t canId, uint8_t *pdata, uint8_t size)
{
  ...
  case 0x2AE:
    ...
    zxdown_buffer[ZXDOWN_A5EE_POS8_ADDR] = pdata[3]; // 中控台输入信息 - 后桥转向角度 ???????????
    tlv_a5ee_valid_flag = 1;
    retval = 0x01;
    break;
    
  case 0x2AF:
    zxdown_buffer[ZXDOWN_A5EE_POS5_ADDR] = pdata[0]; // 中控台输入信息 - 字节状态位3
    zxdown_buffer[ZXDOWN_A5EE_POS6_ADDR] = pdata[1]; // 中控台输入信息 - 字节状态位4
    zxdown_buffer[ZXDOWN_A5EE_POS7_ADDR] = pdata[2]; // 中控台输入信息 - 字节状态位5
    zxdown_buffer[ZXDOWN_A5EE_POS7_ADDR] = pdata[3]; // 中控台输入信息 - 字节状态位6
    tlv_a5ee_valid_flag = 1;
    retval = 0x01;
    break;
...
uint8_t CAN_ProcessRecvEngineMsg(uint32_t canId, uint8_t *pdata, uint8_t size)
{
  ...
  case 0x18FEF100:    
    ...
    if (pdata[6]&BIT(5)) // 巡航控制状态
      zxengine_buffer[ZXENGINE_A5EF_POS14_ADDR] |= BIT(6); // 置1
    else
      zxengine_buffer[ZXENGINE_A5EF_POS14_ADDR] &= ~BIT(6); // 清0
  ...
  case 0x18FD9BA3:
    if (pdata[0]&BIT(0)) // 尿素品质传感器温度(SPN 3515)
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] |= BIT(0); // 置1
    else
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] &= ~BIT(0); // 清0

    if (pdata[3]&BIT(0)) // 品质温度传感器FMI (SPN 3519)
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] |= BIT(1); // 置1
    else
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] &= ~BIT(1); // 清0
    
    if (pdata[4]&BIT(0)) // 品质传感器FMI (SPN3520)
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] |= BIT(2); // 置1
    else
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] &= ~BIT(2); // 清0
    
    if (pdata[5]&BIT(0)) // 催化剂试剂类型(SPN3521)
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] |= BIT(3); // 置1
    else
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] &= ~BIT(3); // 清0
  ...
  case 0x18FE56A3:
    if (pdata[4]&BIT(0)) // 尿素箱液位传感器失效模式FMI
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] |= BIT(4); // 置1
    else
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] &= ~BIT(4); // 清0
    
    if (pdata[5]&BIT(0)) // 尿素箱温度传感器失效模式FMI
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] |= BIT(5); // 置1
    else
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] &= ~BIT(5); // 清0
  ...
  case 0x18FEDF3D:
    if (pdata[7]&BIT(0)) // 尿素箱温度传感器失效模式FMI
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] |= BIT(6); // 置1
    else
      zxengine_buffer[ZXENGINE_A5F1_POS13_ADDR] &= ~BIT(6); // 清0
  ...
  case 0x18FD7C00:
    ...
    if (pdata[2]&BIT(0)) // DPF再生禁止状态
      zxengine_buffer[ZXENGINE_A5F2_POS5_ADDR] |= BIT(2); // 置1
    else
      zxengine_buffer[ZXENGINE_A5F2_POS5_ADDR] &= ~BIT(2); // 清0  
  ...
  case 0x18E00021:
    if (pdata[5]&BIT(2)) // DPF再生开关状态
      zxengine_buffer[ZXENGINE_A5F2_POS5_ADDR] |= BIT(3); // 置1
    else
      zxengine_buffer[ZXENGINE_A5F2_POS5_ADDR] &= ~BIT(3); // 清0  
    
    if (pdata[5]&BIT(0)) // DPF再生禁止开关状态
      zxengine_buffer[ZXENGINE_A5F2_POS5_ADDR] |= BIT(4); // 置1
    else
      zxengine_buffer[ZXENGINE_A5F2_POS5_ADDR] &= ~BIT(4); // 清0  
