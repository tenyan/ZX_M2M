...
void ADC_Initialize(void)
{
  ...
  /// Configure ADC1 Channel12 and Channel13 pin as analog input
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2 | GPIO_Pin_3;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
  GPIO_Init(GPIOC, &GPIO_InitStructure); // PC2和PC3
  ...
  ADC_RegularChannelConfig(ADC1, ADC_Channel_12, 1, ADC_SampleTime_112Cycles); // VRAW_MEAS  ADC_SampleTime_112Cycles
  ADC_RegularChannelConfig(ADC1, ADC_Channel_13, 2, ADC_SampleTime_112Cycles); // VBAT_MEAS
...
void RING_WAKE_GPIO_EXTI_Initialize(void)
{
  ...
  SYSCFG_EXTILineConfig(EXTI_PortSourceGPIOD,EXTI_PinSource12); // Connect EXTI Line12 to PD12 pin
...
void ACC_IN_GPIO_EXTI_Initialize(void)
{
  ...
  // Configure PD15 pin as input floating 
  GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_15;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
  GPIO_Init(GPIOD,&GPIO_InitStructure);
  
  SYSCFG_EXTILineConfig(EXTI_PortSourceGPIOD,EXTI_PinSource15); // Connect EXTI Line15 to PD15 pin
...
{
  if (EXTI_GetITStatus(EXTI_Line12) != RESET) // RING_WAKE_UP
  {
  ...
  if (EXTI_GetITStatus(EXTI_Line15) != RESET) // ACC_IN
  {
