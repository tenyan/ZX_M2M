...
void Tbox_CheckSleepStatus(void)
{
  ...
  vraw = (uint16_t) (adc_dma_buffers[ADC_CHANNEL_VRAW] * 121 / 125 + 67);  // 单位是0.01V
...
void tbox_power_on(void)
{
  ...
  if (tbox_started_delay == FALSE)
  {
    ...
    //RCV_485();           // RS485处于接收状态
    //DISABLE_CAN2_STDY(); // 禁止CAN2休眠
    //DISABLE_CAN1_STDY(); // 禁止CAN1休眠

    Modem_SetState(MODEM_STATE_POWER_ON); // 4G模块开机
    tbox_10msec_timer = 300; // 延时5S,用于自检
  }
  else
  {
...
uint32_t enter_sleep_timer_sp = 30000; // 5分钟
...
void tbox_sleep(void)
{
  ...
  case TBOX_SLEEP_STATE_LIGHT: // 轻度休眠(不断电,RTC一秒唤醒一次)
    ...
    if (tbox_started_delay == FALSE)
    {
      ...
      TBOX_DELAY(OS_TICKS_PER_SEC*40);  // 保证模块正常关机
      PcDebug_SendString("Tbox:L-Sleep!\n");
      tbox_10msec_timer = 1000;
