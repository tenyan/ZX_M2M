...
//uint8 aCANParaData[2048];
extern uint8 auCanParam[2048];
//-----外部变量定义------------------------------------------------------------
...
extern uint8 Version[5];

uint16 g_usSTMCU_Ver = 0x0000;          //默认STMCU版本为0000
//-----内部变量定义------------------------------------------------------------
...
** 日    期: 2020-5-08
******************************************************************************/
uint16 DealSerCmd_SetCANPara(STUSYSPactHeader head,uint16 usCmdTypeLen,uint8 *pCmdType,uint16 usCmdBodyLen,uint8 *pCmdBody)
{
	uint16 usSendLen,usTemp;
	uint8 ucTlvNum;
	uint8 flag = 1;            //初始为失败
	...
	ucTlvNum = *p;
	if(ucTlvNum<=5&&usCmdBodyLen<=1460)
	{
       // flag = 0;
       auCanParam[0] = 0x7E;
	   auCanParam[1] = (uint8)(usCmdBodyLen>>8);
	   auCanParam[2] = (uint8)(usCmdBodyLen&0xFF);
       memcpy(&auCanParam[3],p,usCmdBodyLen);    //考虑长度为0的情况
	   flag = CANConfig_extract_Param();            //初始化参数函数 且返回值
	   //根据返回值判断是否需要保存数据
	   if(flag==0)
	   {
	        ucSumValue = SumCalc(auCanParam, usCmdBodyLen+3);   //校验得到校验和
	        auCanParam[usCmdBodyLen+3] = ucSumValue;  
    	    WriteToFlash(FLASH_CAN_FILE_CFG,usCmdBodyLen+4, auCanParam);//保存到data第一个存储区
            system("sync");
        	sleep(1);
	   }
	}
...
/******************************************************************************
** 函数名称: DealSerCmd_QueryCANPara
** 功能描述: 处理服务器下发的"CR"指令,查询CAN高频采集配置参数
** 输    入: PtrTxt,下发数据包指针; 
** 输    出: PtrTxt,响应数据包指针
** 返    回: 响应数据包长度
** 作    者: lxf
** 日    期: 2020-5-22
******************************************************************************/
uint16 DealSerCmd_QueryCANPara(STUSYSPactHeader head,uint16 usCmdTypeLen,uint8 *pCmdType,uint16 usCmdBodyLen,uint8 *pCmdBody)
{
	uint16 usSendLen,usTemp;
	uint8 ucTlvNum;
	uint8 flag = 1;            //初始为失败
	uint8 *p =  pCmdBody;
	uint8 ucSumValue = 0;
  //  PC_SendDebugData((uint8 *)("QueryCAN"), 8, DEBUG_ANYDATA);
	...
	usSendLen = CANConfig_Read_CanParam(&aMsgSendData[MSG_HEAD_LEN+8]);
	aMsgSendData[MSG_HEAD_LEN+6] = (uint8)(usSendLen>>8);     //?
	aMsgSendData[MSG_HEAD_LEN+7] = (uint8)(usSendLen&0xFF);     //?
    usSendLen += 8;
...
uint16 PackParaData(uint16 usParaId, uint8 *pBuf)
{
	...
	switch(usParaId)
	{
		...
		case 0x0116://硬件版本号，如V1.5表示为0x0105
			...
			*pBuf++ = 0;
			*pBuf++ = 2;
			*pBuf++ = (g_usSTMCU_Ver>>8)& 0xff;
			*pBuf++ =  g_usSTMCU_Ver    & 0xff;
			usParaLen += 6;
			break;	
...
void AllParamSave()
{
	...
	*p = SumCalc(Public_Buf, len+3);
	len += 4;
	WriteToFlash(FLASH_PAGEADDR_PARAMET1,len, Public_Buf);//保存到flash第一个存储区
	usleep(10);
	WriteToFlash(FLASH_PAGEADDR_PARAMET2,len, Public_Buf);//保存到flash第二个存储区
    system("sync");
	usleep(10);
...
void SYS_ParamRead(void)
{
    ...
    ReadFromFlash(FLASH_PAGEADDR_PARAMET1,1024,aParamData);    //从存储器中读取参数的标志和长度 
    ...
    if((aParamData[0]==0xaa) && (usParamLen<=1021))       	//读取的数据标志正确
    {
        if(aParamData[usParamLen+3]==SumCalc(aParamData, usParamLen+3))      //判定读取参数校验和是否正确
        {
			...
			while(ucTlvNum)
			{
				...
				usParaLen = (*p<<8) + *(p+1);
				p += 2;
				if(ParsePara(usParaId, usParaLen, p, 1))
				{
					p += usParaLen;
				}
				else
	...
	if(0==flag)
	{
		...
		ReadFromFlash(FLASH_PAGEADDR_PARAMET2,1024,aParamData);    //从存储器中读取参数的标志和长度 
	    usParamLen = (aParamData[1]<<8)+aParamData[2];     //参数长度
	    if((aParamData[0]==0xaa) && (usParamLen<=1021))       	//读取的数据标志正确
	    {
	        if(aParamData[usParamLen+3]==SumCalc(aParamData, usParamLen+3))      //判定读取参数校验和是否正确
	        {
				...
				WriteToFlash(FLASH_PAGEADDR_PARAMET1,1024, Public_Buf);
				flag = 1;
			}
	    }
		if(0==flag)
...
void SysParaToFactory()
{
	...
	WriteToFlash(FLASH_PAGEADDR_PARAMET1,1, temp);
	WriteToFlash(FLASH_PAGEADDR_PARAMET2,1, temp);
}
...
uint16 DealSerCmd_RemoteCtrl(STUSYSPactHeader head,uint16 usCmdTypeLen,uint8 *pCmdType,uint16 usCmdBodyLen,uint8 *pCmdBody)
{
	...
	switch(usTemp)
	{
		...
		case 0xA510:     //绑定、解绑指令、强制解绑
			...
			else if(2==ucTemp&&usLen==5)  //强制解绑
			{
			    p++;
                m_stuMcuCmd.ucECURemoveBindingFlag = 1;
				memcpy(&m_stuMcuCmd.aucECURemoveBinding,p,4);
			}
			else 
			{
				ucFlag = 1;
		...
		case 0xA511:     //锁车、解锁指令
		    ...
		    break;
		case 0xA512:     //设置发动机及后处理数据上传
            ucTemp = *p++;
			if(usLen==1)
			{
			    m_stuMcuCmd.ucEngineUpEnable = ucTemp;
				Data_SaveToMemory(FLASH_EngineUpEnable_ADDR1, 1, p);
				Data_SaveToMemory(FLASH_EngineUpEnable_ADDR2, 1, p);
			}
		    break;
		case 0xA513:     //车辆VIN设定
		    ucTemp = *p;
			if(ucTemp==0)
			{
        		m_stuMcuCmd.ucVinValidFlag = 0x00;    // 关闭
        		m_stuMcuCmd.ucVinLen = 0x00;
				Data_SaveToMemory(FLASH_VIN_ADDR1, 1, p);
			}
			else if(ucTemp==1)
			{
                if(*p < VIN_BUFFER_MAX_SIZE)
                {
                    m_stuMcuCmd.ucVinValidFlag = 1;
                    m_stuMcuCmd.ucVinLen = *p++;
					memcpy(&m_stuMcuCmd.aucVin,p,m_stuMcuCmd.ucVinLen);
				    Data_SaveToMemory(FLASH_EngineUpEnable_ADDR1, m_stuMcuCmd.ucVinLen+2, p-2);
				}
			}
		    break;
		default:
...
** 日    期: 2020-11-28
******************************************************************************/
void Data_SaveToMemory(char *Pathname ,uint16 usLen, uint8 *p)
{
	uint8 aucTemp[DATA_SaveTOMemory_MAX_SIZE];

    if(usLen > DATA_SaveTOMemory_MAX_SIZE-4)
		return;
	
	aucTemp[0] = 0x12;
	aucTemp[1] = 0x34;
	aucTemp[2] = usLen;
	memcpy(&aucTemp[3],p,usLen);
	aucTemp[usLen+3] = SumCalc(aucTemp, usLen+3);

	WriteToFlash(Pathname,DATA_SaveTOMemory_MAX_SIZE,aucTemp);
	OSTimeDly(OS_TICKS_PER_SEC/200);
}

/******************************************************************************
** 函数名称: Data_ReadFromMemory
** 功能描述: 处理服务器下发的"命令请求"
** 输    入: Pathname,存储文件路径             
             usLen，有效数据长度
             *p，数据指针
...
** 日    期: 2020-11-28
******************************************************************************/
uint8 Data_ReadFromMemory(char *Pathname ,uint16 usLen, uint8 *p)
{
	uint8 aucTemp[DATA_SaveTOMemory_MAX_SIZE] = {0};

    if(usLen > DATA_SaveTOMemory_MAX_SIZE-4)
		return;

	ReadFromFlash(Pathname,DATA_SaveTOMemory_MAX_SIZE, aucTemp);
	if(aucTemp[0] == 0x12&&aucTemp[1] == 0x34)
	{
        usLen = aucTemp[2];
		if(aucTemp[usLen+3]==SumCalc(aucTemp, usLen+3))
		{
            p = &aucTemp[3];
			return usLen;
		}
...
uint16 DealSerCmd_CmdReq(uint8 * PtrTxt, STUSYSPactHeader head)
{
	...
	if((usCmdTypeLen>0) && (usCmdTypeLen<=head.usMsgBodyLen))
	{
		...
		else if(0==memcmp(pCmdType, "CF", 2))   //CAN高频采集参数配置
		{
            return DealSerCmd_SetCANPara(head, usCmdTypeLen, pCmdType, usCmdBodyLen, pCmdBody);
		}
		else if(0==memcmp(pCmdType, "CR", 2))  //CAN高频采集参数查询
		{
            return DealSerCmd_QueryCANPara(head, usCmdTypeLen, pCmdType, usCmdBodyLen, pCmdBody);
		}
		else
		{
...
uint16 DealSerCmd_UpgradeNote(STUSYSPactHeader head,uint16 usCmdTypeLen,uint8 *pCmdType,uint16 usCmdBodyLen,uint8 *pCmdBody)
{
	...
	if((usParaId==0x100c) && (usParaLen>0) && (usParaLen<50))
	{
		...
		if(FirmwareUpdate.ucdev==0&&(memcmp(p, "AUX_MCU", 7)!=0)&&(memcmp(p,"WLRC_WJ",7)!=0))      //判断终端升级时固件名称起始字符
		{
			return 0;
		}
		else
		{
			if(memcmp(p,"AUX_MCU",7)==0)         //4G终端协处理器
				FirmwareUpdate.ucdev = 3;

			FirmwareUpdate.ucSWNameLen = usParaLen;
    		memcpy(FirmwareUpdate.aucSWName, p, usParaLen);

			if(memcmp(p,"WLRC_WJ",7)==0)   //模块4G
			{
			    memcpy(Version, &p[FirmwareUpdate.ucSWNameLen-3], 3); //获取版本号最后面3个数字
			//printf("Version=%s",Version);             //tBOXWJ_4G_190327_V001
			}			
		}
	}
	else
	...
	*p++ = 0;
	  ...
	  )
	{
		...
		//GSM_LinkReConnet(0);
		GSM_SetModemWorkingState(2);   //重新创建socket
	}
	FirmwareUpdate.usTimeoutCnt = 120;	//1分钟没有连接上就算失败
...
uint16 DealSerCmd_UpgradeQuery(STUSYSPactHeader head,uint16 usCmdTypeLen,uint8 *pCmdType,uint16 usCmdBodyLen,uint8 *pCmdBody)
{
	...
	{
    	system("rm -f /data/Auxi_MCU");           //删除原有协处理器升级文件
    	system("rm -f /media/card/Vehicle_MCU");  //删除原有外部MCU器升级文件
	}
	system("sync");
	sleep(1);
...
uint32 GetCrc32(void)
{
	...
	/*
	    ...
	    {
	    	IWdtFeed();
			if(FirmwareUpdate.ucdev==0)  //终端升级
...
uint32 GetCrc32(void)
{
	...
	for(k=0; k<nDataLen; k++)
	{
	    ...
	    if(0==k%FIRMWARE_PACKET_LEN)
	    {
	        //    lseek(FLASH_FIRMWARE_Core_MCU,1000,SEEK_SET);
         //喂狗
			if(FirmwareUpdate.ucdev==0)        //终端4G应用程序升级
			    ReadFromWholFileFlash(FLASH_FIRMWARE_Core_MCU, FIRMWARE_PACKET_LEN, &atempbff[0]);
			else if(FirmwareUpdate.ucdev==1)   //车辆控制器升级
			    ReadFromWholFileFlash(FLASH_FIRMWARE_Vehicle_MCU, FIRMWARE_PACKET_LEN, &atempbff[0]);
			else if(FirmwareUpdate.ucdev==3)   //辅助处理器
			    ReadFromWholFileFlash(FLASH_FIRMWARE_Auxi_MCU, FIRMWARE_PACKET_LEN, &atempbff[0]);
		}	
		crc = (crc >> 8) ^ crctab[(crc & 0xFF) ^ atempbff[n]];
		n++;
		if(n==FIRMWARE_PACKET_LEN)
...
uint16 DealSerCmd_UpgradeLoad(STUSYSPactHeader head,uint16 usCmdTypeLen,uint8 *pCmdType,uint16 usCmdBodyLen,uint8 *pCmdBody)
{
	...
	if(FirmwareUpdate.ucdev==0)
	    WriteCanToFlash(FLASH_FIRMWARE_Core_MCU , usPackLen, p);
	else if(FirmwareUpdate.ucdev==1)
	    WriteCanToFlash(FLASH_FIRMWARE_Vehicle_MCU, usPackLen, p);
	else if(FirmwareUpdate.ucdev==3)
		WriteCanToFlash(FLASH_FIRMWARE_Auxi_MCU, usPackLen, p);
	...
	if(FirmwareUpdate.usRequestPacketSn==FirmwareUpdate.usPackets)   //传输结束
	{
		...
		if(uiTemp==FirmwareUpdate.uiCrc)
		{
			...
			if(FirmwareUpdate.ucdev)
			    stu_McuFirmware.usMcuUPdateTimerOut = FirmwareUpdate.usPackets*3;
...
uint16 DealSerCmd_UpgradeLoad(STUSYSPactHeader head,uint16 usCmdTypeLen,uint8 *pCmdType,uint16 usCmdBodyLen,uint8 *pCmdBody)
{
	...
	if(FirmwareUpdate.usRequestPacketSn==FirmwareUpdate.usPackets)   //传输结束
	{
		...
		if(uiTemp==FirmwareUpdate.uiCrc)
		{
		    ...
		    if(FirmwareUpdate.ucdev)
		    {
                stu_ExtMCUUpgrade.ucUpgradeStep = 1;
				if(FirmwareUpdate.ucdev==3)       //协处理器
				    stu_ExtMCUUpgrade.usTimeOut = 180;     //3分钟
				else if(FirmwareUpdate.ucdev==1)  //外部控制器
					stu_ExtMCUUpgrade.usTimeOut = 660;     //11分钟
				PC_SendDebugData((uint8 *)("ExtMCULoadOK"), 12, DEBUG_ANYDATA);
			}
			else
			    PC_SendDebugData((uint8 *)("WLRCLoadOK"), 10, DEBUG_ANYDATA);
		}
		else
		{
			FirmwareUpdate.ucRet = 1;
...
uint16 DealSerCmd_UpgradeRet(STUSYSPactHeader head,uint16 usCmdTypeLen,uint8 *pCmdType,uint16 usCmdBodyLen,uint8 *pCmdBody)
{
	...
	if((0==*p) && (0==FirmwareUpdate.ucRet))
	{
		...
		if(stu_McuFirmware.ucLoadStep==7&&FirmwareUpdate.ucdev)  //判断如果是协处理器或者控制器器升级应答处理,不需要重启4G模块程序
		{
            stu_McuFirmware.ucLoadStep = 0;
			stu_McuFirmware.ucRcvPackflag = 0;
			stu_ExtMCUUpgrade.ucUpgradeStep = 1;
...
uint16 DealSerCmd_UpgradeRet(STUSYSPactHeader head,uint16 usCmdTypeLen,uint8 *pCmdType,uint16 usCmdBodyLen,uint8 *pCmdBody)
{
	...
	{
		FirmwareUpdate.ucStep = 0;
		if(stu_ExtMCUUpgrade.ucUpgradeStep==3&&FirmwareUpdate.ucdev)  //判断如果是协处理器或者控制器器升级应答处理,不需要重启4G模块程序
		{
		    stu_ExtMCUUpgrade.ucUpgradeStep = 0;  //升级流程结束
         //   stu_McuFirmware.ucLoadStep = 0;
		//	stu_McuFirmware.ucRcvPackflag = 0;
		//	stu_ExtMCUUpgrade.ucUpgradeStep = 1;
			return 0;
...
uint16 DealSerCmd(uint8* PtrTxt,uint16 usPtrTxtLen,uint8 ucSrcAddr,uint8 ucKind)
{
	...
	{
		PC_SendDebugData((uint8 *)("ERR:011"), 7, DEBUG_GPRS);
    ...
    {
    	PC_SendDebugData((uint8 *)("ERR:013"), 7, DEBUG_GPRS);
		return 0;
    }
	usLen -= 1;	//去校验
	...
	{
		PC_SendDebugData((uint8 *)("ERR:014"), 7, DEBUG_GPRS);
		return 0;
	}
