 ...
 ******************************************************************************/
volatile uint16_t dbg_uart_rx_size;
...
void DBG_UartInitialize(uint32_t baudrate)
{
  ...
  PcDebug_fd = open(DEV_TTYGS0_CDC_DEBUG, O_RDWR|O_NOCTTY); // open uart 
  if (PcDebug_fd < 0)
  {
    printf("ERROR open %s ret=%d\n\r", DEV_TTYGS0_CDC_DEBUG, PcDebug_fd);
    close(PcDebug_fd);
    PcDebug_fd = -1;
    return ;
  }
  ...
  tcflush(PcDebug_fd, TCIFLUSH);
  tcsetattr(PcDebug_fd, TCSANOW, &options);
}
...
uint8_t DBG_UartReceiveData(uint8_t **data, uint16_t* size);
{
  ...
  ret = read(PcDebug_fd, dbg_uart_rx_buffer, (DBG_UART_RX_BUFFER_SIZE-1)); // 阻塞等待数据
  if (ret > 0)
  {
...
uint16_t DBG_UartTransmitData(uint8_t *data,uint16_t size);
{
  ...
  if ((0==size) || (NULL==data) || (PcDebug_fd<0))
  {
    return 0;
  }
 
  retVal = write(PcDebug_fd, data, size); // 写串口数据
  if (retVal != size)
  {
