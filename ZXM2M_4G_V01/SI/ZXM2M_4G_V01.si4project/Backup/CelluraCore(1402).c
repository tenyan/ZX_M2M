 ...
 *   Macros
 ******************************************************************************/
#define CELLURA_Receive    USART3_ReceiveData

#define CELLULAR_MAX_RXBUF_SIZE  USART3_RX_BUFFER_SIZE
...
#define AT_DELAY(ms)         do { osDelay(ms); } while(0)
#define MODEM_DELAY_MS(ms)   do { osDelay(ms); } while(0)
#define CELLURA_DELAY(ms)    do { osDelay(ms); } while(0)
...
volatile uint8_t current_open_conId = 0xFF;
volatile uint8_t current_send_conId = 0xFF;
volatile uint8_t current_close_conId = 0xFF;
 ...
 ******************************************************************************/
//=消费者==============================================================
#define AppThreadPriority_CelluraProcess   osPriorityHigh2
osThreadId_t tid_CelluraProcess;

const osThreadAttr_t AppThreadAttr_CelluraProcess =
{
  .priority = AppThreadPriority_CelluraProcess,
  .attr_bits = osThreadDetached,
  .stack_size = 2048, // 字节
};

 //=生产者==============================================================
#define AppThreadPriority_CelluraProduce   osPriorityHigh1
osThreadId_t tid_CelluraProduce;

const osThreadAttr_t AppThreadAttr_CelluraProduce =
{
  .priority = AppThreadPriority_CelluraProduce,
  .attr_bits = osThreadDetached,
  .stack_size = 2048, // 字节
};
...
const char AT_QCCID[]="AT+QCCID\r\n"; // 获取SIM卡的ICCID
#define SIZE_OF_AT_QCCID  (sizeof(AT_QCCID)/sizeof(AT_QCCID[0]) - 1)
...
const char AT_QCFG3[]="AT+QCFG=\"nwscanmode\",0\r\n"; // 设置网络搜索模式为Automatic(LTE/WCDMA/GSM)
#define SIZE_OF_AT_QCFG3 (sizeof(AT_QCFG3)/sizeof(AT_QCFG3[0]) - 1)
...
#define SIZE_OF_AT_QIACT  (sizeof(AT_QIACT)/sizeof(AT_QIACT[0]) - 1)

const char AT_QIDEACT[]="AT+QIDEACT=1\r\n"; // 去激活PDP场景
#define SIZE_OF_AT_QIDEACT  (sizeof(AT_QIDEACT)/sizeof(AT_QIDEACT[0]) - 1)

const char AT_CGPADDR[]="AT+CGPADDR=1\r\n"; // 查询PDP地址
#define SIZE_OF_AT_CGPADDR  (sizeof(AT_CGPADDR)/sizeof(AT_CGPADDR[0]) - 1)

const char AT_QSCLK[]="AT+QSCLK=1\r\n"; // 查询PDP地址
#define SIZE_OF_AT_QSCLK  (sizeof(AT_QSCLK)/sizeof(AT_QSCLK[0]) - 1)

void Modem_SocketClose(skt_context_t* pThis);
...
{
  USART3_TransmitData(pdata, size);
...
atStatus_t AT_SendCmd(at_cmd_t atcmdId, uint8_t retries, uint16_t timeout)
{
  ...
  case AT_CMD_QCCID_GET: // 获取SIM卡的ICCID
    at_buf = AT_QCCID;
    at_size = SIZE_OF_AT_QCCID;
    break;
...
{
  //等待所有socket关闭
  if(net_IsAllSocketClosed()==SOCKET_FALSE)
  {
    return;
  }

  PcDebug_SendString("Modem Sleep\n");
  
  // 去激活PDP,只发一次
  AT_SendCmd(AT_CMD_QIDEACT_SET, 3, OS_TICKS_PER_SEC);
 
  // 禁止网络状态和小区码自动上报
  AT_SendCmd(AT_CMD_CREG0_SET, 3, (OS_TICKS_PER_SEC/20));

  // 使能模块休眠
  AT_SendCmd(AT_CMD_QSCLK, 3, (OS_TICKS_PER_SEC/20));

  MODEM_DELAY_MS(OS_TICKS_PER_SEC);
  
  MODEM_DTR_H(); // 使模块进入休眠模式

  Modem_SetState(MODEM_STATE_SLEPT);
}
...
{
  PcDebug_SendString("Modem Wakeup\n");
  MODEM_DTR_L(); // 使模块退出休眠模式
}
...
void Modem_StateManageService(void)               /// 阻塞
{
  ...
  case MODEM_STATE_POWER_ON:  // 模块开机
    Modem_PowerOn();
    Modem_SetState(MODEM_STATE_INIT); // 转初始化
    break;
  ...
  case MODEM_STATE_SLEEPING:  // 模块休眠中
    Modem_Sleep();
    break;

  case MODEM_STATE_SLEPT:  // 模块已休眠
    break;

  case MODEM_STATE_WAKEUP:  // 唤醒模块
    Modem_Wakeup();
    Modem_SetState(MODEM_STATE_INIT); // 4G模块初始化
    break;  
...
** 返    回: 0=成功,1=失败
*******************************************************************************/
void Modem_SocketCreat(skt_context_t* pThis, uint8_t conId, uint8_t srv_protocol, uint8_t *srv_ip, uint8_t srv_ip_size, uint16_t srv_port)
{
  pThis->conId = conId;
  pThis->srv_protocol = srv_protocol;
  memset(pThis->srv_ip,0x00,MAX_IP_ADDR_SIZE);
  memcpy(pThis->srv_ip,srv_ip,srv_ip_size);
  pThis->srv_port = srv_port;
}
...
** 返    回: 0=成功,1=失败
*******************************************************************************/
uint8_t Modem_SocketConnect(skt_context_t* pThis)
{
  uint8_t it;
  uint8_t buff[70] = {0};

  if (pThis->conId >= CELLURA_MAX_CONNS)
    return 1;

  if (pThis->srv_protocol==CS_TCP_PROTOCOL)
  {
    sprintf((char*)buff, "AT+QIOPEN=1,%d,\"TCP\",\"%s\",%d,0,1\r\n", pThis->conId,pThis->srv_ip, pThis->srv_port);
  }
  else if (pThis->srv_protocol==CS_UDP_PROTOCOL)
  {
    sprintf((char*)buff, "AT+QIOPEN=1,%d,\"UDP\",\"%s\",%d,0,1\r\n", pThis->conId,pThis->srv_ip, pThis->srv_port);
  }

  current_open_conId = pThis->conId;
  atcmd_rsp_status = ATCMD_RSP_NONE;
  AT_SET_CMD_ID(AT_CMD_QIOPEN);
  AT_transaction(buff, strlen((char*)buff)); // 通过串口DMA发送AT指令到模块
  for (it=0; it<200; it++) // 最长等待20秒
  {
    AT_DELAY(OS_TICKS_PER_SEC/10); // 每100ms判断一次
    if (ATCMD_RSP_OK == atcmd_rsp_status)
    {
      break;
    }

    if (ATCMD_RSP_NOK == atcmd_rsp_status)
    {
      break;
    }
  }

  current_open_conId = 0xff;
  AT_SET_CMD_ID(AT_CMD_IDLE);
  if (ATCMD_RSP_OK == atcmd_rsp_status)
  {
 ...
 * 功能描述: 发送数据处理函数
*******************************************************************************/
void Modem_SocketSend(skt_context_t* pThis)
{
  at_buf_t at_buff[30];
  uint8_t at_size;
  uint8_t it,j;

  if ((1==pThis->new_tx_data_flag)) // 是否有新数据待发送
  {
    pThis->link_state = SOCKET_LINK_STATE_BUSY;
    
    for (it=0; it<2; it++) // 重发两次
    {
      sprintf((char*)at_buff, "AT+QISEND=%d,%d\r", pThis->conId, pThis->tx_len);

      current_send_conId = pThis->conId;
      atcmd_rsp_status = ATCMD_RSP_NONE;
      AT_SET_CMD_ID(AT_CMD_QISEND);  // 发送AT指令
      at_size = strlen((char*)at_buff);
      AT_transaction(at_buff, at_size);
      for (j=0; j<200; j++) // 等1s
      {
        AT_DELAY(OS_TICKS_PER_SEC/200); // 检测周期: 5ms
        if (ATCMD_RSP_OK == atcmd_rsp_status)
        {  break;}
      }
      current_send_conId = 0xff;
      AT_SET_CMD_ID(AT_CMD_IDLE);

      if (ATCMD_RSP_OK == atcmd_rsp_status) // 写入数据
      {
        current_send_conId = pThis->conId;
        atcmd_rsp_status = ATCMD_RSP_NONE;
        AT_SET_CMD_ID(AT_CMD_QISEND_DATA);  // 发送用户数据
        AT_transaction(pThis->tx_buff, pThis->tx_len);
        for (j=0; j<250; j++) // 等待5s
        {
          AT_DELAY(OS_TICKS_PER_SEC/50); // 检测周期20ms
          
          if (ATCMD_RSP_OK == atcmd_rsp_status)
          {  break;}
          else if (ATCMD_RSP_NOK == atcmd_rsp_status)
          {  break;}
        }
        current_send_conId = 0xFF;
        AT_SET_CMD_ID(AT_CMD_IDLE);
        ...
        {
          pThis->error_cnt++;
          PcDebug_SendString("AT_QIDATA: Fail\n");
        }
      }
      else
      {
        pThis->error_cnt++;
        PcDebug_SendString("AT_QISEND: Fail\n");
      }
    }// end for()
    pThis->new_tx_data_flag = 0; // 注意数据丢失
    pThis->link_state = SOCKET_LINK_STATE_READY;
  } // end if ((1==pThis->new_tx_data_flag))
}
...
void Modem_SocketClose(skt_context_t* pThis)
{
  ...
  sprintf((char *)at_buff, "AT+QICLOSE=%d\r", pThis->conId);
  for (i=0; i<3; i++) // 重发3次
  {
    current_close_conId = pThis->conId;
    atcmd_rsp_status = ATCMD_RSP_NONE;
    AT_SET_CMD_ID(AT_CMD_QICLOSE);
    AT_transaction(at_buff, strlen((const char *)at_buff));
    for (j=0; j<100; j++) // 超时时间10S
    {
      AT_DELAY(OS_TICKS_PER_SEC/10); // 每100ms检查一次
      if (ATCMD_RSP_OK == atcmd_rsp_status)
      {
        current_close_conId = 0xff;
        AT_SET_CMD_ID(AT_CMD_IDLE);
        return;
      }
    }
...
*******************************************************************************/
void AppThread_CelluraProduce(void *argument)
{
  while (1)  // 10m执行一次
  {
...
/* ================================================================== */
uint8_t AT_RspAnalyze_QIOPEN(uint8_t *pdata, uint16_t size)
{
  // 直吐模式 +QIOPEN: <connectID>,<err>
  // 如有错误 ERROR
  const char *str;
  uint8_t connectID = 0xFF;
  uint8_t error_code = 0xFF;

  str = strnstr((char *)pdata, size, "+QIOPEN", 7);
  if (str!=NULL)
  {
    str += 9;
    connectID = AT_ParseNumber(&str);
    error_code = AT_ParseNumber(&str);

    // 当服务成功打开,<err>是 0,其他情况下<err>不是 0
    if(current_open_conId==connectID && (error_code==0))
    {  
      return 1;
    }
  }
  ...
  {
    str = strnstr((char *)pdata, size, "ERROR", 5);
    if (str!=NULL)
    {
      return 2;
    }
...
//uint8_t AT_RspAnalyze_QISEND_DATA(uint8_t *pdata, uint16_t size)
 ...
 * CAT1模块数据接收进程
*******************************************************************************/
void AppThread_CelluraProcess(void *argument)
{
  uint8_t *pdata = NULL;
  uint16_t len;
...
{
  tid_CelluraProcess = osThreadNew(AppThread_CelluraProcess, NULL, &AppThreadAttr_CelluraProcess);  
  tid_CelluraProduce = osThreadNew(AppThread_CelluraProduce, NULL, &AppThreadAttr_CelluraProduce);
}
