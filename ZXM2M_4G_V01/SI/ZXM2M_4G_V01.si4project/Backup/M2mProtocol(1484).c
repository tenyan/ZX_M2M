...
#define im2m_GetLinkState()    net_socket_GetLinkState(&m2m_socket)
#define im2m_ReconnectLink()   do{net_socket_SetLinkState(&m2m_socket, SOCKET_LINK_STATE_CLOSED);\
                                  m2m_context.conn_success_flag = M2M_FALSE;}while(0)
#define im2m_SendNetData(pData, len)  do{net_socket_send(&m2m_socket, pData, len);}while(0)
...
uint16_t im2m_AnalyzeCmdReqLtMsg(m2m_context_t* pThis)
{
  ...
  if (pThis->rx_from != SYSBUS_DEVICE_TYPE_PC_DEBUG) // 根据平台下发设置追踪模式
  {
...
void M2M_ProduceSendMsg(m2m_context_t* pThis)
{
  ...
  if ((pThis->ss_req.send_timer==0x00) && (pThis->ss_req.sent_flag==M2M_FALSE))
  {
    ...
    pThis->ss_req.rsp_timeout_timer = pThis->conn_req.rsp_timeout_sp;
...
* 下面函数由用户编写,用于底层网络连接
*******************************************************************************/
void ReInitM2mServerAddr(void)
{
  m2m_socket.srv_protocol = m2m_asset_data.main_srv_protocol;
  m2m_socket.srv_port = m2m_asset_data.main_srv_port;
  sprintf((char *)m2m_socket.srv_ip, "%d.%d.%d.%d",m2m_asset_data.main_srv_ip[0],m2m_asset_data.main_srv_ip[1],m2m_asset_data.main_srv_ip[2],m2m_asset_data.main_srv_ip[3]);
}
...
{
  m2m_socket.conId = SOCKET_LINK_M2M;
  m2m_socket.state = NET_SOCKET_STATE_INVALID;
  m2m_socket.started_delay = SOCKET_FALSE;
  m2m_socket.state_transition = SOCKET_TRUE;
  m2m_socket.error_cnt = 0;
  m2m_socket.error_flag = SOCKET_FALSE;
  m2m_socket.enable_flag = SOCKET_TRUE;
  m2m_socket.link_state = SOCKET_LINK_STATE_CLOSED;

  m2m_socket.tx_buff = m2m_conn_tx_buffer;
  m2m_socket.tx_len = 0;
  m2m_socket.new_tx_data_flag = 0;

  m2m_socket.reinit_server_addr = &ReInitM2mServerAddr;
  (*m2m_socket.reinit_server_addr)();
}
