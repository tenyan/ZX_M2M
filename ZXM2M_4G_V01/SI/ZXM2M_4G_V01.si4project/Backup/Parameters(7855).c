...
void Parm_SaveLvcInfo(void)
{
  ...
  sfud_write(&sfud_w25q256fv, EMAP_LVC_INFO1_ADDRESS, SIZEOF_LVC_CONTEXT, (uint8_t *)&lvc_context);
  PARM_DELAY(2); // 延时2ms
  sfud_write(&sfud_w25q256fv, EMAP_LVC_INFO2_ADDRESS, SIZEOF_LVC_CONTEXT, (uint8_t *)&lvc_context);
}
...
void Parm_ReadLvcInfo(void)
{
  ...
  sfud_read(&sfud_w25q256fv, EMAP_LVC_INFO1_ADDRESS, SIZEOF_LVC_CONTEXT, (uint8_t *)&lvc_ctx1); // 读取主备份
  ...
  sfud_read(&sfud_w25q256fv, EMAP_LVC_INFO2_ADDRESS, SIZEOF_LVC_CONTEXT, (uint8_t *)&lvc_ctx2); // 读取副备份
  ...
  if (main_is_ok==PARM_TRUE) // 主备份OK
  {
    ...
    if (redundant_is_ok==PARM_FALSE)
    {
      ...
      sfud_erase(&sfud_w25q256fv,EMAP_LVC_INFO2_ADDRESS,EMAP_LVC_INFO2_SIZE); // 擦除
      PARM_DELAY(1); // 延时1ms
      sfud_write(&sfud_w25q256fv, EMAP_LVC_INFO2_ADDRESS, SIZEOF_LVC_CONTEXT, (uint8_t *)&lvc_context);
    }
  ...
  if (redundant_is_ok==PARM_TRUE) // 副备份OK
  {
    ...
    if (main_is_ok==PARM_FALSE)
    {
      ...
      sfud_erase(&sfud_w25q256fv,EMAP_LVC_INFO1_ADDRESS,EMAP_LVC_INFO1_SIZE); // 擦除
      PARM_DELAY(1); // 延时1ms
      sfud_write(&sfud_w25q256fv, EMAP_LVC_INFO1_ADDRESS, SIZEOF_LVC_CONTEXT, (uint8_t *)&lvc_context);
    }
    return;
...
void Parm_SaveTotalWorkTimeInfo(void)
{
  ...
  sfud_erase(&sfud_w25q256fv,EMAP_WORK_TIME_INFO1_ADDRESS,EMAP_WORK_TIME_INFO1_SIZE); // 擦除
  PARM_DELAY(2); // 延时2ms
  sfud_erase(&sfud_w25q256fv,EMAP_WORK_TIME_INFO2_ADDRESS,EMAP_WORK_TIME_INFO2_SIZE); // 擦除
  PARM_DELAY(2); // 延时2ms
  ...
  sfud_write(&sfud_w25q256fv, EMAP_WORK_TIME_INFO1_ADDRESS, 7, buf);
  PARM_DELAY(2); // 延时2ms
  sfud_write(&sfud_w25q256fv, EMAP_WORK_TIME_INFO2_ADDRESS, 7, buf);
}
...
void Parm_ReadTotalWorkTimeInfo(void)
{
  ...
  sfud_read(&sfud_w25q256fv, EMAP_WORK_TIME_INFO1_ADDRESS, 7, m_buf); // 读取主备份
  if (Parm_CalcXorValue(m_buf, 6) == m_buf[6] && (m_buf[0] == 0xA5)&&(m_buf[1] == 0x5A))
  {
    main_is_ok = PARM_TRUE;
  }

  sfud_read(&sfud_w25q256fv, EMAP_WORK_TIME_INFO2_ADDRESS, 7, r_buf); // 读取副备份
  ...
  if (main_is_ok==PARM_TRUE) // 主备份OK
  {
    ...
    if (redundant_is_ok==PARM_FALSE)
    {
      ...
      sfud_erase(&sfud_w25q256fv,EMAP_WORK_TIME_INFO2_ADDRESS,EMAP_WORK_TIME_INFO2_SIZE); // 擦除
      PARM_DELAY(1); // 延时1ms
      sfud_write(&sfud_w25q256fv, EMAP_WORK_TIME_INFO2_ADDRESS, 7, m_buf);
    }
    return;
  }
  ...
  if (redundant_is_ok==PARM_TRUE) // 副备份OK
  {
    ...
    if (main_is_ok==PARM_FALSE)
    {
      ...
      sfud_erase(&sfud_w25q256fv,EMAP_WORK_TIME_INFO1_ADDRESS,EMAP_WORK_TIME_INFO1_SIZE); // 擦除
      PARM_DELAY(1); // 延时1ms
      sfud_write(&sfud_w25q256fv, EMAP_WORK_TIME_INFO1_ADDRESS, 7, r_buf);
    }
    return;
  }
...
void Parm_SaveM2mAssetData(void)
{
  ...
  sfud_erase(&sfud_w25q256fv,EMAP_M2M_ASSET_DATA1_ADDRESS,EMAP_M2M_ASSET_DATA1_SIZE); // 擦除
  PARM_DELAY(2); // 延时2ms
  sfud_erase(&sfud_w25q256fv,EMAP_M2M_ASSET_DATA2_ADDRESS,EMAP_M2M_ASSET_DATA2_SIZE); // 擦除
  PARM_DELAY(2); // 延时2ms
  ...
  sfud_write(&sfud_w25q256fv, EMAP_M2M_ASSET_DATA1_ADDRESS, len, pbuf); // 写入
  PARM_DELAY(2); // 延时2ms
  sfud_write(&sfud_w25q256fv, EMAP_M2M_ASSET_DATA2_ADDRESS, len, pbuf); // 写入
}
...
void Parm_ReadM2mAssetData(void)
{
  ...
  sfud_read(&sfud_w25q256fv, EMAP_M2M_ASSET_DATA1_ADDRESS, 1024, pbuf); // 读取主备份
  header = pbuf[pos++]; // 校验头
  header <<= 8;;
  header += pbuf[pos++];
  ...
  if (parm_is_ok == PARM_FALSE)
  {
    ...
    sfud_read(&sfud_w25q256fv, EMAP_M2M_ASSET_DATA2_ADDRESS, 1024, pbuf); // 读取副备份
    header = pbuf[pos++]; // 校验头
    header <<= 8;;
    header += pbuf[pos++];
    len = pbuf[pos++]; // 长度(包含校验头2B+长度2B)
    ...
    if ((header==0xA55A) && (len <= 1024)) // 校验头和参数长度
    {
      ...
      if (xor_value==pbuf[len]) // 校验值
      {
        ...
        parm_is_ok = PARM_TRUE;
        sfud_erase(&sfud_w25q256fv,EMAP_M2M_ASSET_DATA1_ADDRESS,1024); // 擦除
        PARM_DELAY(2); // 延时2ms
        sfud_write(&sfud_w25q256fv, EMAP_M2M_ASSET_DATA1_ADDRESS, 1024, pbuf); // 写入
      }
    }
...
{
  sfud_erase(&sfud_w25q256fv,EMAP_HEX_FILE_ADDRESS,EMAP_HEX_FILE_SIZE); // 擦除
}
...
void rfu_SaveFlashHexFile(rfu_context_t* pThis, uint8_t *buf, uint16_t length)
{
  ...
  {
    sfud_write(&sfud_w25q256fv, (EMAP_HEX_FILE_ADDRESS + pThis->cumulated_address), length, buf);
...
{
  sfud_read(&sfud_w25q256fv, (EMAP_HEX_FILE_ADDRESS + address), cnt, buf);
}
