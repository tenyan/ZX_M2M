...
{
  //f_stuGsmState.ucLinkState[ZXEP_LINK_NUM] = SOCKET_LINK_STATE_CLOSED;
...
void NetSocket_Invalid(skt_context_t* pThis)
{
  ...
  else
  {
    if (pThis->socket_10ms_timer == 0)
    {
      ...
      {
        pThis->socket_state = NET_SOCKET_STATE_CREATE;
...
void NetSocket_Create(skt_context_t* pThis)
{
...
#endif
      pThis->error_cnt++;
      pThis->socket_state = NET_SOCKET_STATE_CLOSE;
  ...
  else
  {
    if (pThis->socket_10ms_timer == 0)
    {
      pThis->socket_state = NET_SOCKET_STATE_CONNECT;
      pThis->state_transition = SOCKET_TRUE;
    }
  }
}
...
void NetSocket_Connect(skt_context_t* pThis)
{
  ...
  else
  {
    if (pThis->socket_10ms_timer == 0)
    {
      pThis->socket_state = NET_SOCKET_STATE_READY;
      pThis->state_transition = SOCKET_TRUE;
    }
  }
}
...
void NetSocket_Close(skt_context_t* pThis)
{
  ...
  else
  {
    if (pThis->socket_10ms_timer == 0)
    {
      ...
      {
        pThis->socket_state = NET_SOCKET_STATE_SILENCE; // 重连
        pThis->state_transition = SOCKET_TRUE;
      }
      else
      {
        pThis->socket_state = NET_SOCKET_STATE_INVALID; // 重连
        pThis->state_transition = SOCKET_TRUE;
      }
    }
  }
