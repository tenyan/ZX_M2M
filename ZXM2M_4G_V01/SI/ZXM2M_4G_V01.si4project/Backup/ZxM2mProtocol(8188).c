...
uint16_t iZxM2m_BuildTlvMsg_3004(uint8_t *pbuf)
{
  ...
  if (tlv_3004_valid_flag)
  {
    pbuf[len++] = 0x30; // TAG
    pbuf[len++] = 0x04;
    pbuf[len++] = (SIZE_OF_ZXINFO_3004>>8) & 0xFF; // LENGTH
    pbuf[len++] = SIZE_OF_ZXINFO_3004 & 0xFF;
    memcpy(&pbuf[len], zxinfo_buffer_3004, SIZE_OF_ZXINFO_3004); // VALUE
    len += SIZE_OF_ZXINFO_3004;
  }
...
uint16_t iZxM2m_BuildTlvMsg_3005(uint8_t *pbuf)
{
  ...
  if (tlv_3005_valid_flag)
  {
    pbuf[len++] = 0x30; // TAG
    pbuf[len++] = 0x05;
    pbuf[len++] = (SIZE_OF_ZXINFO_3005>>8) & 0xFF; // LENGTH
    pbuf[len++] = SIZE_OF_ZXINFO_3005 & 0xFF;
    memcpy(&pbuf[len], zxinfo_buffer_3005, SIZE_OF_ZXINFO_3005); // VALUE
    len += SIZE_OF_ZXINFO_3005;
  }
