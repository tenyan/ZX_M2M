...
typedef enum
{
  ...
  AT_CMD_QCCID_GET,   //!< 获取SIM卡的ICCID
  AT_CMD_CMGF_SET,    //!< 设置短信格式
  AT_CMD_CPMS_SET,    //!< 设置短信存储位置
  AT_CMD_CNMI_SET,    //!< SMS事件上报配置:不上报
  AT_CMD_QCFG3_SET,   //!< 设置网络搜索模式为Automatic(LTE/WCDMA/GSM)
  ...
  AT_CMD_QIACT_SET,   //!< 激活PDP场景
  AT_CMD_QIDEACT_SET, //!< 去激活PDP场景
  AT_CMD_CGPADDR_GET, //!< 查询PDP地址

  AT_CMD_QSCLK,       //!< 设置模块进入休眠模式
  AT_CMD_NETWORK_ATTACH, //!< Attach to a network
  AT_CMD_NETWORK_DETACH, //!< Detach from network
  AT_CMD_CFUN_SET,       //!< Set Phone Functionality
  AT_CMD_CFUN_GET,       //!< Get Phone Functionality

  AT_CMD_QIOPEN,      //!< Start Up TCP or UDP Connection
  AT_CMD_QISEND,      //!< Send Data Through TCP or UDP Connection
  AT_CMD_QISEND_DATA, //!< Send Data
  AT_CMD_QICLOSE,     //!< Close TCP or UDP Connection
