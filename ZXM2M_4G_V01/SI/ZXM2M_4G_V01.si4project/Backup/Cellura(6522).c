...
atStatus_t AT_SendCmd(at_cmd_t atcmdId, uint8_t retries, uint16_t timeout)
{
  ...
  case AT_CMD_CRESET: // 模块软件复位
   ...
   case AT_CMD_CRESET: // 模块进入飞行模式
     at_buf = AT_CFUN0_SET;
     at_size = SIZE_OF_AT_CFUN0_SET;
     break;
     
   case AT_CMD_CRESET: // 模块为全功能(online mode)
     at_buf = AT_CFUN1_SET;
  ...
  case AT_CMD_ATE0: // 开启GPS会话
    at_buf = AT_CGPS_SET;
    at_size = SIZE_OF_AT_CGPS_SET;
    break;

  case AT_CMD_ATE0: // 使能GPS XTRA
    at_buf = AT_CGPSXE_SET;
    at_size = SIZE_OF_AT_CGPSXE_SET;
    break;

  case AT_CMD_ATE0: // 配置NMEA-0183输出周期5S,输出GPGGA和GPRMC信息
  ...
  case AT_CMD_CGPADDR_GET: // 查询PDP地址
   ...
   case AT_CMD_IPADDR_GET: // 查询Socket PDP地址
    at_buf = AT_IPADDR;
    at_size = SIZE_OF_AT_IPADDR;
    break;
...
uint8_t Modem_Init(void)
{
  ...
  // 查询Socket PDP的IP地址
  at_rsp = AT_SendCmd(AT_CMD_IPADDR_GET, 20, OS_TICKS_PER_SEC);
  if (at_rsp == ATCMD_RSP_NOK)
  {
    return AT_FALSE; // 退出重试
  }
  ...
  // 关闭WIFI模块
  at_rsp = AT_SendCmd(AT_CMD_CWMAP0_SET, 3, OS_TICKS_PER_SEC);
  if (at_rsp == ATCMD_RSP_NOK)
  {
    return AT_FALSE; // 退出重试
  }
...
static void Cellura_ProcessRecvData(uint8_t *pdata, uint16_t size)
{
  ...
  if (urc_flag == CELLULAR_FALSE)
  {
    ...
    case AT_CMD_CGPADDR_GET:  // 查询PDP地址
      ...
      break;
    case AT_CMD_IPADDR_GET:  // 查询Socket PDP地址
      if (AT_RspAnalyze_IPADDR(pdata, size))
      {
        atcmd_rsp_status = ATCMD_RSP_OK;
      }
      break;
